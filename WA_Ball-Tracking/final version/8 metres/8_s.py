...
cap1 = cv2.VideoCapture('a108.mp4')   # import the video file of behind the goal
cap2 = cv2.VideoCapture('a08.mp4')   # import the video file of the corner of the pool 
...
def send(x,y,x_od,y_od,g,px,py):
    d={1: x, 2: y, 3: x_od, 4: y_od, 5: g, 6: px, 7: py}
    msg =pickle.dumps(d)
...
while True:
    ...
    while(cap1.isOpened()):
        ...
        detector = cv2.SimpleBlobDetector_create(blob)     # apply the parameters to detector 

        
        reversemask1 = 255-mask1
        #cv2.imshow("Reverse Mask", reversemask)
        keypoints1 = detector.detect(reversemask1)         #use the simple blob function to return keypoints
        
        
        reversemask2 = 255-mask2
        #cv2.imshow("Reverse Mask", reversemask)
        keypoints2 = detector.detect(reversemask2)         #use the simple blob function to return keypoints
        ...
        xb = get_x_keypoints(keypoints1)           # use the get keypoints functions to set the variable values
        yb = get_y_keypoints(keypoints1)
        xs = get_x_keypoints(keypoints2)
        ys = get_y_keypoints(keypoints2)

        xb = realiser(xb)                          # use the realiser function to ensure there is no "None" type responses
        if xb > 0:                                 # set the current value to reading value if the current value is none zero
                xba = xb
        yb = realiser(yb)
        if yb > 0: 
                yba = yb
        ...
        g = glt(xba, yba, xsa, ysa)                # send the coordinates to the glt function, returns a 0 for no goal and 1 for goal
        
        beta = (109*(1-(xba/1352)) + 35)*0.0174533      # angle calculations for the balls coordinates
        alpha = (90 - (78*xsa/1352 + 51))*0.0174533



        a = math.tan(alpha)
        b = math.tan(beta)
        y = 10 - (8 - 10*a)/(b-a)                       # coordinate outputs
        x = (10*a-8)/(a/b - 1)
        ...
        x_od = x*30 + 50                    #CALIBRATE THE POOL OUTPUT DIAGRAM FOR X AND Y
        y_od = 650-(y*30)

        xbb=750*xba/977 + 50               #CALIBRATE THE GOAL OUTPUT DIAGRAM FOR X AND Y, 977 is far goal post coordinate
        ybb=300*(yba)/760 + 50

        send(x,y,x_od,y_od,g,xbb,ybb)       # use the send function to send the data to the output diagram script

        
        # these lines change the colour of the ball if it is detected in both cameras
        if xb > 0 and xs > 0:       
        ...
        # show the frame with the keypoint circles drawn on
        ...
        # this line allows for the program to be exitted during the script
